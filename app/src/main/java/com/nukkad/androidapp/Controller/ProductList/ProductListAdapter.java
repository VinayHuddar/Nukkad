...
import com.nukkad.androidapp.View.ProductDetailActivity;
...
/**
 * Created by vinayhuddar on 28/08/15.
 */
public class ProductListAdapter extends ArrayAdapter implements QuantityUpdateCallbacks {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        mCurrOption = currProduct.GetOptionSelection();

        LayoutInflater inflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        final View listItem;
        if (convertView == null) {
            listItem = inflater.inflate(R.layout.layout_product_data_list_view, null);
        } else {
        ...
        String label;
        if (currProduct.GetManufacturerName() != null)
            label = String.format("%s: %s", currProduct.GetManufacturerName(), currProduct.GetName());
        else
        ...
        imageView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra(IntentActionStrings.PRODUCT_ID, (int)getItemId(pos));
                intent.putExtra(IntentActionStrings.CATEGORY_ID, mCategoryId);
                intent.putExtra(IntentActionStrings.OPTION_SELECTION, getItem(pos).GetOptionSelection());
                intent.putExtra(IntentActionStrings.SELLER_ID, currProduct.GetProductData().GetSellerId());
        ...
        if (currProduct.GetProductData().GetSellerId() == CommonDefinitions.INVALID_ID)
            listItem.findViewById(R.id.SellerField).setVisibility(View.GONE);
        else {
            TextView seller = (TextView)listItem.findViewById(R.id.Seller);
            seller.setPaintFlags(seller.getPaintFlags() | Paint.UNDERLINE_TEXT_FLAG);
            seller.setText(currProduct.GetProductData().GetSellerName());
            seller.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(mContext, SellerProfileActivity.class);
                    intent.putExtra(IntentActionStrings.SELLER_ID, currProduct.GetProductData().GetSellerId());
                    intent.putExtra(IntentActionStrings.FROM_PRODUCT_DETAIL_VIEW, false);
                    mContext.startActivity(intent);
    ...
    void UpdateOptionBasedViews (ProductListManager.ProductDataForListView prod, View listItem) {
        ...
        int qty = currProduct.GetQuantityInCart(mCurrOption);
        if (qty > 0) {
        ...
        if (currProduct.GetSpecial(mCurrOption) != null) {
            // Set Old Price
            oldPriceTV.setPaintFlags(oldPriceTV.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
            oldPriceTV.setText(currProduct.GetPrice(mCurrOption));

            // Set Special Price
            currentPriceTV.setText(currProduct.GetSpecial(mCurrOption));
        } else {
            // Set current price
            currentPriceTV.setText(currProduct.GetPrice(mCurrOption));
        }
    }
